...
function check_build() {
   cargo build
    ...
    then
      ...
      echo ":rotating_light: Try running **./cargo build** in the local dpab_core folder to learn more." >> "${COMMENT_FILE}"
      exit 1
    fi
...
{
  grcov . -s src --binary-path ./target/debug/ -t lcov --branch --ignore-not-existing --llvm --ignore "*cargo*" --ignore "*target*" --ignore "*model*" --ignore "*dpab_appsanity*" -o ./target/debug/lcov.info
